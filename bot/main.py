...
async def safe_answer_callback(c: CallbackQuery, text: str = "", show_alert: bool = False):
    try:
...
async def gate_recheck(c: CallbackQuery, bot: Bot):
    if await pass_gate(bot, c.from_user.id):
        ...
        await safe_answer_callback(c, "Still not joined both.", show_alert=True)
...
async def product_view(c: CallbackQuery):
    ...
    if not p or p["is_active"] != 1:
        await safe_answer_callback(c, "Product unavailable.", show_alert=True)
        return
...
async def product_buy(c: CallbackQuery, bot: Bot):
    ...
    user = await db.user_by_tg(c.from_user.id)
    if not user:
        await safe_answer_callback(c, "User not found.", show_alert=True)
        return

    bundle, err = await db.create_order_with_unit(user["id"], product_id)
    if err:
        await safe_answer_callback(c, err, show_alert=True)
        return
    ...
    await safe_answer_callback(c, "Purchase complete ✅", show_alert=True)
...
async def order_view(c: CallbackQuery):
    ...
    if not o:
        await safe_answer_callback(c, "Order not found.", show_alert=True)
        return
...
async def order_show_again(c: CallbackQuery, bot: Bot):
    ...
    if not unit:
        await safe_answer_callback(c, "No delivery unit found.", show_alert=True)
        return
    ...
    await safe_answer_callback(c, "Re-delivered ✅", show_alert=True)
...
async def order_dispute(c: CallbackQuery):
    ...
    if not dispute_allowed(o["delivered_at"], settings.dispute_window_hours):
        await safe_answer_callback(c, "Dispute window expired.", show_alert=True)
        return
