...
# simple in-memory state for dispute reason collection
...
async def wallet_deposit(c: CallbackQuery):
    await c.message.answer(
        "➕ Deposit request:\n"
        "Manual methods currently enabled (as per settings): UPI / Binance ID.\n"
...
async def text_router(m: Message, bot: Bot):
    ...
    if m.from_user.id in PENDING_DISPUTE_REASON:
        ...
        # notify support chat if configured
...
async def menu_coupons(c: CallbackQuery):
    await c.message.answer("🎟 Coupon apply flow comes in next batch.")
    await c.answer()
